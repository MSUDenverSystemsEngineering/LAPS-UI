...
Try {
	...
	[string]$appName = 'LAPS - UI'
	[string]$appVersion = '6.2'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '09/08/2021'
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Show-InstallationWelcome -CloseApps 'AdmPwd.UI' -CloseAppsCountdown 60
		...
		$exitCode = Execute-MSI -Action "Uninstall" -Path "$dirFiles\LAPS.x64.msi" -Parameters "REBOOT=ReallySuppress /QN" -PassThru
		If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
